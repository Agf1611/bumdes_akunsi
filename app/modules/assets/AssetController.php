...
final class AssetController extends Controller
{
    ...
    public function index(): void
    {
        try {
            ...
            $this->view('assets/views/index', [
                ...
                'rows' => $rows,
                'summary' => $summary,
                'auditSummary' => $this->model()->getAuditSummary(),
    ...
    private function showAssetForm(string $title, ?array $row): void
    {
        ...
        $formData = [
            ...
            'acquisition_cost' => old('acquisition_cost', isset($row['acquisition_cost']) ? (string) ((float) $row['acquisition_cost'] == floor((float) $row['acquisition_cost']) ? (string) (int) round((float) $row['acquisition_cost']) : rtrim(rtrim(number_format((float) $row['acquisition_cost'], 2, '.', ''), '0'), '.')) : '0'),
            'opening_as_of_date' => old('opening_as_of_date', (string) ($row['opening_as_of_date'] ?? date('Y-m-d'))),
            'opening_accumulated_depreciation' => old('opening_accumulated_depreciation', isset($row['opening_accumulated_depreciation']) ? (string) ((float) $row['opening_accumulated_depreciation'] == floor((float) $row['opening_accumulated_depreciation']) ? (string) (int) round((float) $row['opening_accumulated_depreciation']) : rtrim(rtrim(number_format((float) $row['opening_accumulated_depreciation'], 2, '.', ''), '0'), '.')) : '0'),
            'residual_value' => old('residual_value', isset($row['residual_value']) ? (string) ((float) $row['residual_value'] == floor((float) $row['residual_value']) ? (string) (int) round((float) $row['residual_value']) : rtrim(rtrim(number_format((float) $row['residual_value'], 2, '.', ''), '0'), '.')) : '0'),
