...
<form method="get" action="<?= e(base_url('/ledger')) ?>" class="row g-3 align-items-end">
    ...
    <div class="col-lg-2"><label class="form-label">Periode Awal</label><select name="period_id" class="form-select"><?= report_period_select_options($periods, (int) ($filters['period_id'] ?? 0), 'Manual tanggal') ?></select></div>
    <div class="col-lg-2"><label class="form-label">Sampai Periode</label><select name="period_to_id" class="form-select"><?= report_period_select_options($periods, (int) ($filters['period_to_id'] ?? 0), 'Sama dengan periode awal') ?></select></div>
